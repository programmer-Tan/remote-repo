...
class myQMainWindow(QMainWindow):
    ...
    def clock(self):
        ...
        if sender.text() == '签到':
            ...
            try:
                ...
                QMessageBox.critical(self, '错误', f'训练模型出现错误：{e}')
    ...
    def show_camera1(self):
        ...
        # todo 签到
        if self.ui.PushButton_2.isEnabled() == False and 1 == len(faces):
            ...
            try:
                ...
                self.ui.label_7.setStyleSheet("color:#00a000;font-weight: bold;"
                                              "font-size: 20px;font-family:system-ui;text-align:justify")
                ...
                self.ui.label_6.setText("unknown")
                self.ui.label_3.clear()

        # todo 签退
        elif self.ui.PushButton_3.isEnabled() == False and 1 == len(faces):
            showImage1 = self.taking_pictures(faces_ls=faces_ls, img=img_no)
            showImage1 = QImage2CV(showImage1)
            try:
                img, self.name = recognize(showImage1,self.model,self.names)
